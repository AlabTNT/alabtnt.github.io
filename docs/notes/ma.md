...
[2024SS](https://zju-turing.github.io/TuringCourses/math_phys/math_analysis2/%E6%95%B0%E5%AD%A6%E5%88%86%E6%9E%90%EF%BC%88%E7%94%B2%EF%BC%89%E2%85%A1%EF%BC%88H%EF%BC%892024%E6%98%A5%E5%A4%8F%E5%9B%9E%E5%BF%86%E5%8D%B7.pdf) | [Answer](./2023-2024MaAnAnswer.pdf)
...
1. **二元函数微分定义**(P102)：设函数 $ z=f(x,y) $ 在点 $ P_0(x_0,y_0) $ 的某邻域 $ U(P_0) $ 上有定义，对于 $ U(P_0) $ 中的点 $ P(x,y)=(x+0+\Delta x,y_0+\Delta y) $ ，若函数 $ f $ 在点 $ P_0 $ 处的全增量 $ \Delta z $ 可表示为

<span id="s1"></span>

 $$ \Delta z=f(x+0+\Delta x,y_0+\Delta y)-f(x_0,y_0) \newline =A\Delta x+B\Delta y+\omicron(\rho) -->(1) $$ 
...
<span id="s2"></span>
 ...
 $$ dz|_{P_0}=df(x_0,y_0)=A\Delta x+B\Delta y -->(2) $$ 
...
1. 函数项级数一致收敛的 Cauchy 准则：函数项级数 $ \Sigma u_n(x) $ 在数集 $ D $ 上一致收敛的充要条件为：对任给的正数 $ \epsilon $ ，总存在某正整数 $ N $ ，使得当 $ n>N $ 时，对一切 $ x\in D $ 和一切正整数 $ p $ ，都有
...
(ii) 对于每一个 $ x\in I, {v_{n}(x)} $ 是单调的;  
(iii) 在 $ I $ 上 $ v_n(x)\rightrightarrows 0(n\rightarrow\inf) $   
则级数 $ \Sigma u_{n}(x)v_{n}(x) $ 在 $ I $ 上一致收敛.
...
$$f(x) ~ \frac{a_0}{2} + \Sigma^{\inf}_{n=1}(a_n cos nx + b_n sin nx)$$

其中

$$a_n=\frac{1}{\pi}\int^{\pi}_{-\pi}f(x)cos nxdx \newline b_n=\frac{1}{\pi}\int^{\pi}_{-\pi}f(x)sin nxdx$$
...
$$f(x) ~ \frac{a_0}{2} + \Sigma^{\inf}_{n=1}(a_n cos \frac{n\pi x}{l} + b_n sin {n\pi x}{l})$$

其中

$$a_n=\frac{1}{l}\int^{l}_{-l}f(x)cos \frac{n\pi x}{l}dx \newline b_n=\frac{1}{l}\int^{l}_{-l}f(x)sin \frac{n\pi x}{l}dx$$
...
$$\int_L f(x,y)ds=\int_{a}^{b} f(x,\psi(x))\sqrt{1+\psi'^2(t)}dx. $$
...
14. 链式法则：$ \frac{\partial f(u,v)}{\partial x} = \frac{\partial f}{\partial u}\cdot\frac{\partial u}{\partial x} + \frac{\partial f}{\partial v}\cdot\frac{\partial v}{\partial x}$
...
1. $ 1+sinx = (sin\frac{x}{2}+cos\frac{x}{2})^2 $ 

<span id="byd2.0"></span>

2. 第一二型曲线积分：对于一段曲线， $ ds $ 就是一型，$ 一堆dx + 另一堆dy $ 就是二型。计算方法：把 $ x $ 和 $ y $ 替换成 $ t $ 。如果 $ x $ 和 $ y $ 已经有等量关系了，直接令自变量为 $ t $ 。然后带入原式子求积分就行了。积分上下限取得是 $ t $ 的可求范围

<span id="byd3.0"></span>

3. 第一二型曲面积分：对于一块曲面， $ dS $ 就是一型， $ dxdy $ 就是二型。一型只要把 $z$ 带进去再乘 $\sqrt{1+z_x^2+z_y^2}$ 即可。二型只要把 $z$ 带进去即可。**注意二型有时出现 $ dz $，需要用Jacobi换元。**

<span id="byd4.0"></span>

4. Jacobi 常用两换元：  
二元换元：令 $ x=rcos\theta, y=rsin\theta $; 三元换元：令 $ x=rsin\theta cos\phi, y=rsin\theta sin\phi, z=cos\theta $
 $$ dS=rdrd\theta \newline dV=r^2 sin\theta drd\theta d\phi $$ 
二元先积 $ r $ 再积 $ \theta $ ，三元先积 $ r $ 再积 $ \theta $ 最后积 $ \phi $ 

5. $ F(x,y,...)=\sqrt{(x-x_0)^2+(y-y_0)^2+...}$ 时，有 $ \frac{\partial F}{\partial x} = \frac{x-x_0}{F(x,y,...)} $

6. $ \Sigma_{k=1}^{n}coskx=\frac{1}{2\sin{\frac{x}{2}}}(\sin{(n+\frac{1}{2})x}-\sin{\frac{x}{2}})\leq\frac{1}{2\sin{\frac{x}{2}}}-1$
