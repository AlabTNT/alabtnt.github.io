...
宝牌狂热：和出的牌宝牌较多的情况。一般宝牌大于6或里宝牌大于4即可认为是一次“宝牌狂热”。

宝牌指示牌：在王山上翻开用于指示宝牌的牌，这张牌的“下一张牌”作为本局的宝牌。详见[宝牌、里宝牌、红宝牌](./2.0.md/#dora_intro)

### C

场{揃|jiān}：指计算番数时默认起始番数。M规（或是基本所有日麻规则）的场揃为2番。这也就代表着计算点数时要计算 $a = 符数^(番数+2) $。
...
<span id="jiantao"></span>

\*见逃：在其他家立直、副露后明显听牌，并且根据场况分析可以认为他们的牌可能非常大时，选择放弃和牌的行为。例如一家只打过筒子和索子，那么他很有可能是万子清一色或混一色。这时的牌可能要在4000点以上，而在流局时自己最多付3000，警觉家自摸需要付的更少，而自己的牌如果很差或离听牌很远，那么如果要听牌的话在路上很有可能放铳，因此有必要放弃和牌转而防守，以免放铳。
...
闲家：每局游戏不是庄家的即为闲家。闲家可以不必在意游戏进程，可以见逃。
